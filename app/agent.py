"""
The production agent — the s10 full agent with the ACCESS-CONTROL guardrail folded in.

s10 wired router -> specialists -> structured tools + unified RAG, fully Langfuse-traced. This is the
same graph, with one difference that was previously a separate guardrails/ script: the tool dispatch
now enforces least-privilege on WHOSE data. Every tool call whose `customer_id` argument names an
account other than the authenticated caller is BLOCKED before the tool runs — deterministic,
model-independent. This is the single source of truth for the agent; the app layers memory and the
output guardrail around classify() in app/pipeline.py.
"""
...
# least-privilege on WHICH tools (s4): each specialist gets its structured tools + the knowledge tool.
SPECIALIST_TOOLS = {
...
# ══ ACCESS-CONTROL GUARDRAIL (folded in from guardrails/access_control) ═════════════════════════
# The tool dispatch takes the AUTHENTICATED customer id and blocks any tool call whose `customer_id`
# argument names a different account. Deterministic, model-independent — the leak becomes impossible.
def _run_tool(name, args, allowed, auth_customer_id):
    if name not in allowed:                                    # least-privilege on WHICH tools (s4)
        return {"error": f"tool {name} is not available to this specialist"}
    requested = args.get("customer_id")
    if requested and auth_customer_id and requested != auth_customer_id:      # least-privilege on WHOSE data
        return {"error": f"access_denied: this agent may only access the authenticated account "
                         f"'{auth_customer_id}'. Refusing to access '{requested}'."}
...
def make_specialist(category: str):
    ...
    def node(state: State) -> dict:
        if category not in state["route"]:
            return {}
        auth_id = state["context"].get("customer_id")          # the authenticated caller
        ...
        for _ in range(MAX_STEPS if schemas else 0):
            ...
            for tc in msg.tool_calls:
                ...
                result = _run_tool(tc.function.name, args, allowed, auth_id)     # ← access control enforced here
...
def classify(ticket: str, customer_context: dict | None = None) -> dict:
    """Route the ticket, run the chosen specialist(s) with their scoped tools + RAG, aggregate."""

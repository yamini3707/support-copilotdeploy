...
Only once the account is healthy, check get_incident_status (pass the customer's plan and region so
you see only incidents that could affect them). Treat an incident as the cause ONLY if it directly
concerns the same service the customer reported; do not link it indirectly.
""" + _SPECIALIST_TAIL
...
account matter, so set handled false. For a genuine missing feature, tell them it isn't currently
available and that you'll pass the request along as feedback. Never promise it will be built or is
on the roadmap.
""" + _SPECIALIST_TAIL
